...
float findangle(float x,float z,float off, float normal_ang,float amin,float amax,
...
int main (int argc, char **argv)
{
  ...
  char *filename="taco";
	...
	if (!getparfloat("Av_min",&Av_min)) Av_min = 0.001;
	...
	/* compute uniformly sampled cdp */
        for(icdp=0; icdp<ncdp; ++icdp) xcdp[icdp] = fcdp+icdp*dcdp; 
...
/*	icip = (xcdp[0] - fcdp_cig)/dcdp + 1;
	...
	{	
		...
		{	
			...
			ang = findangle(xcip[icip],zoff[ioff],off,a_normal,amin,amax,
			...
			rayinc = makeRay(xcip[icip],zoff[ioff],a_normal+ang,nt,dt,c11,c33,c13,c55,
			...
			rayref = makeRay(xcip[icip],zoff[ioff],a_normal-ang,nt,dt,c11,c33,c13,c55,
				c15,c35,nx,dx,fx,nz,dz,fz,amax,amin);   
			...
			memset((void *) dt1dpara, 0, FSIZE*3*nx*nz);
			memset((void *) dt2dpara, 0, FSIZE*3*nx*nz);
		...
		memset((void *) dzdpara_avg, 0, FSIZE*3*nx*nz);
		
		zoff_avg = 0;
		
		for(ioff=0;ioff<noff_avg;ioff++)
		{
			zoff_avg += zoff[ioff]/noff_avg;
			
			for (iver=0;iver<3*nx*nz;iver++)
			{
				dzdpara_avg[iver] += dzdpara[ioff][iver]/noff_avg;
			}
		}
		...
		{	k0=k;
		        for (iver=0;iver<3*nx*nz;iver++)
			{
				...
				a = sc*(dzdpara[ioff][iver] - dzdpara_avg[iver]);					
				if (fabs(a)>Av_min)
				{	k += 1;
					Av[k] = a;
					Aj[k] = iver+1;			
			...
			{	nn += 1;
				...
				b[ioff+nnoff-noff_avg-nn] = sc*(zoff[ioff] - zoff_avg);
				for (j=k0+1;j<k+1;j++) Ai[j] = ioff+nnoff-noff_avg-nn+1;			
			}
		}
	...
	efwrite(b,sizeof(float),nnoff,bfp);
